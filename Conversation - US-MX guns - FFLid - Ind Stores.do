...
*** Using <append> instead. Possible double counting, but no false duplicate dropping. ***
	duplicates drop year FFLaddress_match serialnumber, force
	merge 1:1 year FFLaddress_match serialnumber using "courtcase_temp.dta"
	...
	* Collapsing to FFL
		...
		* Creating branches and chain variables
		...
		duplicates drop FFLaddress_match, force
			...
			* Harmonize state variable <state> and <statesale>?
		...
		replace branches = 1 if branches == .
		replace chain = 0 if chain == .
		...
		* Replacing FFL codes
		...
		collapse (count) count (first) studystates chain FFLaddress FFLaddress_match FFLcity FFLaddresscity FFLstate, by(FFL)
			...
			labe var FFLstate "State"
			
		gen FFLstate2 = subinstr(FFLstate," ","",.)
		levelsof FFLstate2, local(levels)
		foreach l of local levels {
			display "Processing level: `l'"
			gen count_`l' = 0
			replace count_`l' = count if FFLstate2 == "`l'"
			}
			
		save "CENAPI-USCourt-FFLaddress-collapse.dta", replace

		** Graph for Independents **
		use "CENAPI-USCourt-FFLaddress-collapse.dta", clear
		/*
		drop axis*
		*/
		label drop _all
			gsort + chain - count statesale
			gen axis = _n
			/*
			...
			*/
			...
			graph hbar ///
				count_Arizona count_California count_Florida count_Massachusetts ///
				count_Missouri count_NewMexico count_Ohio count_Oklahoma count_Nevada ///
				count_Texas count_Washington if count > 14 & chain == 0, ///
				over(axis, lab(angle(0) labsize(tiny))) ///
				stack ytitle("Guns Seized") ylab(,labsize(small)) legend(col(3) ///
